...
def logkobling_avledning(innds, resultatds):
    ...
    koblelog = koblelog + "</ul><strong>Resultat: {} med {} observasjoner</strong><ul>".format(resultatds, so_dict[resultatds].count())
    for ds in innds:
        koblelog = koblelog + "<li> Fra {} ble det koblet {} observasjoner</li>".format(ds, so_dict[ds].filter(so_dict[ds].personidentifikator.isin(so_dict[resultatds]['personidentifikator'])).count())
